...
namespace VRCCCMod.CC
{
    ...
    /// </summary>
    public class TranscriptSession : IDisposable
    {
        ...
        private void UpdateRecognizer()
        {
            ...
            try
            {
                var tmp = GameUtils.GetVoiceRecognizer();
                if (tmp != null)
                {
        ...
        public void RunInference()
        {
            ...
            {
                if (ready_for_processing.Count == 0)
                {
                    return;
                }

                if (!ready_for_processing.TryDequeue(out buff))
                {
                    return;
                }
            }
        ...
        public void FlushCurrentAudio()
        {
            lock (ready_for_filling)
            {
                lock (ready_for_processing)
                {
                    ...
                    AudioBuffer buff;
                    if (!ready_for_filling.TryPeek(out buff))
                    {
                        return;
                    }
        ...
        public void CommitSayingIfTooOld()
        {
            ...
            {
                if (Utils.GetTime() - active_saying.timeEnd > sepAge)
                {
                    CommitSaying();
                }
            }
        ...
        private void EnsureAdditionalBuffers()
        {
            lock (ready_for_filling)
            {
                if (ready_for_filling.Count == 0)
                {
                    if (audioBuffers.Count >= maxAudioBuffers)
                    {
                        return;
                    }
                    ...
                    GameUtils.LogDebug("Allocated 1 extra buffer for " + audioSource.GetFriendlyName() + ". Total count: " + audioBuffers.Count);
                }
            }
        }
            ...
            if (audioBuffers.Count > 2)
            {
                lock (ready_for_filling)
                {
                    lock (audioBuffers)
                    {
                        ...
                        var to_remove = new List<AudioBuffer>();
                        foreach (var buff in audioBuffers)
                        {
                            if (buff.queued || buff.buffer_head > 1)
                            {
                                continue;
                            }

                            if (!ready_for_filling.Contains(buff))
                            {
                                continue;
                            }
                        ...
                        {
                            GameUtils.LogDebug("Removed " + to_remove.Count + " buffers from " + audioSource.GetFriendlyName() + ". Total count: " + audioBuffers.Count);
                        }
                    }
                }
            }
        ...
        /// <returns>
        ///   The number of samples that were actually sved,
        ...
        public int EatSamples(float[] samples, int len)
        {
            ...
            lock (ready_for_filling)
            {
                ...
                if (!buff.readWriteMutex.WaitOne(1))
                {
                    return -1;
                }
                ...
                {
                    last_activity = Utils.GetTime();
                    if (buff.buffer_head == AudioBuffer.buffer_size)
                    {
                        return 0;
                    }
        ...
        /// </summary>
        public static float sepAge { get; } = 0.666f;
        ...
        /// </summary>
        public static float maxAge { get; } = 8.0f;
        ...
        {
            if (past_sayings[idx_to_start_from] == null)
            {
                return float.PositiveInfinity;
            }

            if (idx_to_start_from + 1 >= past_sayings.Count)
            {
                return past_sayings[idx_to_start_from].timeEnd;
            }

            for (var i = idx_to_start_from + 1; i < past_sayings.Count; i++)
            {
                if (past_sayings[i - 1] == null || past_sayings[i] == null)
                {
                    return float.PositiveInfinity;
                }

                var prevEnd = past_sayings[i - 1].timeEnd;
                var currStart = past_sayings[i].timeStart;
            ...
            if (past_sayings[past_sayings.Count - 1] == null)
            {
                return float.PositiveInfinity;
            }
        ...
        /// </summary>
        private void UpdateText(bool IsPriority = false)
        {
            CleanUpOldSayings();
            lock (past_sayings)
            {
                textGen.UpdateText(past_sayings, active_saying, IsPriority);
            }
        }

        public static event Action<IAudioSource, string> OnGetText;
        ...
        {
            OnGetText?.Invoke(audioSource, textGen.FullText);
        ...
        public void Update()
        {
            ...
            UpdateText(whitelisted);

            if (ui == null && !disposed)
            {
                ui = new SubtitleUi(this);
            }
